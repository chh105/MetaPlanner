function qi = calc_dose_spill_indices(cst,pln,doseCube,targetRefDose,param)
...
for runVoi = 1:size(cst,1)
    ...
    if ~isempty(doseInVoi)
        ...
        VTarget100 = sum(doseInVoi >= 1*referenceDose); % number of target voxels recieving dose >= 0.95 dPres
        VTreated50 = sum(doseCube(:) >= 0.50*referenceDose);  %number of all voxels recieving dose >= 0.50 dPres ("treated volume")
        VTreated90 = sum(doseCube(:) >= 0.90*referenceDose);  %number of all voxels recieving dose >= 0.90 dPres ("treated volume")
        VTreated100 = sum(doseCube(:) >= 1*referenceDose);  %number of all voxels recieving dose >= 0.90 dPres ("treated volume")
        
%         qi(runVoi).(['CN_100']) = VTarget100/(VTreated100); 
        qi(runVoi).(['R50']) = VTreated50/numOfVoxels; 
                     
        qi(runVoi).(['R90']) = VTreated90/numOfVoxels; 
        
        voiPrint = sprintf('%sR50 = %6.4f, R90 = %5.2f for reference dose of %3.2f Gy\n',voiPrint,...
                           qi(runVoi).(['R50']),qi(runVoi).(['R90']),referenceDose);
