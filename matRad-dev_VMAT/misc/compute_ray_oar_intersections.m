function intersecting_voxels = compute_ray_oar_intersections(ct,cst,pln,visMode)
...
V = [];
for i=1:size(cst,1)
    if isequal(cst{i,3},'OAR') && ~isempty(cst{i,6})
        V = [V;vertcat(cst{i,4}{:})];
    end
end
...
% Define matrix to contain intersecting voxels
intersecting_voxels = zeros(1,length(pln.propStf.gantryAngles));
...
for i = 1:length(pln.propStf.gantryAngles)
    ...
    for j = stf(i).numOfRays:-1:1
        ...
        intersecting_voxels(i) = intersecting_voxels(i) + length(ix);
