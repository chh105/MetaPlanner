function [ct, cst, pln, stf] = import_dicom_resample_to_given_dim( ctDir, rtStDir, in_dims, dicomMetaBool, useDoseGrid )
...
[new_xres, new_yres, new_zres] = calc_resample_res( in_dims(1), in_dims(2), in_dims(3), 512, 512, size(files.ct,1), files.resx, files.resy, files.resz);
...
%import ct-cube
waitbar(1 / steps)
resolution.x = new_xres;
resolution.y = new_yres;
resolution.z = new_zres; % [mm] / lps coordinate system
...
assert(ct.dicomInfo.Width == ct.dicomInfo.Height)
assert(ct.dicomInfo.Width == 512)
...
%     pln.propStf.gantryAngles    = [0,45,90,135,180,225,270,315]; % [?]
    ...
    pln.propDoseCalc.doseGrid.resolution.x = new_xres; % [mm]
    pln.propDoseCalc.doseGrid.resolution.y = new_yres; % [mm]
    pln.propDoseCalc.doseGrid.resolution.z = new_zres; % [mm]
