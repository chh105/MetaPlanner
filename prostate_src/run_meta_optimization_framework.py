    ...
    for i in range(num_cases):
        ...
        for j in range(len(ordered_couch_angle_list)):
            partial_arc_starting_gantry_angle = ordered_gantry_angle_list[j] \
                                                - num_control_points//2*vmat_control_point_spacing
...
# def run_nonuniform_sampling(data_parent_dir,
#                             output_directory,
#                             use_noncoplanar=False):
#     memory = 12000000000 * 9
#     object_store_memory = 2500000000 * 8
...
#         file_path = os.path.join(directory, 'trajectory_dict_for_case_num_' + str(sample_case_num) + '.npy')
#         trajectory_start_time = time.time()
#         trajectory_searcher = coplanarNonuniformSampling([],
#                                                          data_dir=data_parent_dir,
#                                                          sample_case_num=sample_case_num,
#                                                          visualize=True,
#                                                          results_dir=directory,
...
#     print('avg trajectory time per patient:', np.sum(trajectory_time_data) / num_cases)
#
# def run_uniform_sampling(data_parent_dir,
#                             output_directory,
#                             use_noncoplanar=False):
#     memory = 12000000000 * 9
#     object_store_memory = 2500000000 * 8
...
#
#         trajectory_dict = {}
#         couch_angle_list = list(np.zeros(180))
#         gantry_angle_list = list(np.linspace(0,358,180))
#         trajectory_dict['sampled_path_couch_angles'] = couch_angle_list
#         trajectory_dict['sampled_path_gantry_angles'] = gantry_angle_list
#
#         trajectory_time_elapsed = time.time() - trajectory_start_time
#         print('trajectory search time elapsed:', trajectory_time_elapsed)
#         trajectory_search_time_list.append(trajectory_time_elapsed)
...
def load_data_and_make_plots(meta_optimization_results_dir,
    ...
    for i in range(start, num_cases):
        ...
        phys_dict['ci'].append(phys_ci)
        phys_dict['hi'].append(phys_hi)
        phys_dict['r50'].append(phys_r50)
        phys_dict['r90'].append(phys_r90)
        ...
        ci_val, hi_val = matrad_env.get_ci_and_hi()
        ...
        pops_dict['ci'].append(ci_val)
        pops_dict['hi'].append(hi_val)
        ...
        results_dict['physician'] = phys_dict
        results_dict['pops'] = pops_dict
